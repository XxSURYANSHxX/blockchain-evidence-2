 ...
 * Handles loading states, transaction confirmations, and user feedback
 */
...
class BlockchainFeedback {
    ...
    showSkeletonLoader(container, type = 'table', count = 5) {
        if (!container) return;
        ...
        for (let i = 0; i < count; i++) {
            ...
            skeleton.className = `skeleton-loader skeleton-${type}-row`;
            container.appendChild(skeleton);
        }
    }

    hideSkeletonLoader(container, originalContent = '') {
        if (!container) return;
    ...
    showTransactionPending(txHash, message = 'Waiting for blockchain confirmation...') {
        ...
        modal.innerHTML = `
            ...
            <div style="margin: 12px 0;">
                <small style="color: #888;">Transaction Hash:</small><br>
                <a href="${explorerUrl}" target="_blank" class="tx-hash-link">
    ...
    showTimeoutWarning(txHash) {
        const existingModal = document.getElementById(`tx-modal-${txHash}`);
        if (existingModal) {
            const explorerUrl = this.getExplorerUrl(txHash);
            existingModal.innerHTML = `
                <div style="display: flex; align-items: center; margin-bottom: 12px;">
                    <i data-lucide="clock" style="width: 20px; height: 20px; color: #f59e0b; margin-right: 10px;"></i>
                    <strong style="color: #f59e0b;">Transaction Delayed</strong>
                </div>
                <p style="margin: 8px 0; color: #666;">Blockchain may be congested. Please wait or check explorer.</p>
                <div style="margin: 12px 0;">
                    <a href="${explorerUrl}" target="_blank" class="btn btn-sm" style="background: #f59e0b; color: white; text-decoration: none; padding: 6px 12px; border-radius: 4px;">
                        View in Explorer
                    </a>
                </div>
                <button onclick="this.parentElement.remove()" style="position: absolute; top: 8px; right: 8px; background: none; border: none; font-size: 18px; cursor: pointer; color: #999;">×</button>
    ...
    showTransactionSuccess(txHash, message = 'Evidence added successfully!') {
        ...
        this.showToast('success', `
            ...
            <div>
                <div>${message}</div>
                <small>
                    <a href="${explorerUrl}" target="_blank" class="tx-hash-link" style="color: rgba(255,255,255,0.8);">
                        View Transaction
                    </a>
                </small>
            </div>
    ...
    // Utility method for blockchain operations
    ...
    // Mock confirmation waiting (replace with actual blockchain integration)
